...
public class Kabsch : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ApplyKabschTransform();
    }

    private void ApplyKabschTransform()
    {
        for (int i = 0; i < inPoints.Length; i++)
        {
            refPoints[i] = new Vector4(referencePoints[i].position.x, referencePoints[i].position.y,
                referencePoints[i].position.z, referencePoints[i].localScale.x);
        }

        Matrix4x4 kabschTransform = solver.SolveKabsch(points, refPoints, solveScale: true);
...
public class KabschSolver
{
        ...
        bool solveScale = false)
    {
        ...
        if (solveRotation)
        {
            ...
            extractRotation(TransposeMultSubtract(inPoints, refPoints, inCentroid, refCentroid, DataCovariance),
                ref OptimalRotation);
            Profiler.EndSample();
        }
